...
const fitReducer = (draftState: Draft<DiagramMakerWorkspace>, action: FitAction) => {
  ...
  const topPositionOffset = Math.max((viewContainerSize.height - expctedHeight * scale) / 2, 0);
  const leftPositionOffset = Math.max((viewContainerSize.width - expectedWidth * scale) / 2, 0);
  const expectedPosition = { x: (-minX * scale) + leftOffset + leftPositionOffset, y: (-minY * scale) + topPositionOffset };
